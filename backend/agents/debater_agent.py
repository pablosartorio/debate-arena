...
una estrategia, sus 2-3 key_claims, y a qué apuntar como contraargumento.
...
def _system_prompt(persona: Persona) -> str:
    return (
        ...
        "debate. Esta es tu fase privada de razonamiento — el output NO se le "
...
def _user_prompt(
    ...
    parts.append(
        ...
        '}\n'
        "En español rioplatense. Se conciso."
    )
    return "\n".join(parts)
...
async def plan_for_turn(
    ...
    Si falla, devuelve plan vacio (el speak corre sin plan — degradacion elegante).
    """
